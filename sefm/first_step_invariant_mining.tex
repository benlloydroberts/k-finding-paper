...
Recall the data generation process illustrated in Figure \ref{fig:trajectories}. Initially, our observation data set $A_{\tau}$ consists of all stacked agent trajectories, from which invariant properties are to be mined. Invariants may be manifest temporally, as patterns across state sequences, such as whenever we observe $X$ in $S_{t}$, we always observe $Y$ in $S_{t+1}$. Stacking all agent trajectories to form our dataset preserves temporal properties, recorded during many training episodes, for future sequence mining. Similarly, we may infer certain program properties by through a static representation in terms of unique, reachable states. Figure \ref{fig:unique_states} illustrates this for all 127 reachable states given a ladder logic program consisting $2^{20}$ theoretic states. Each row represents a state while each column corresponds to a program variable. Each cell in Figure \ref{fig:unique_states} represents a value (purple : 0, yellow : 1) for the respective variable under a given state. Recall our base pelican crossing program (Figure \ref{fig:trajectories}) concerned variables $CROSSING,...,AUDIO$. We see from the set of reachable states in Figure \ref{fig:unique_states} a candidate invariant disallowing both traffic lights and pedestrian lights to simultaneously display green aspects, i.e $PL\_1_\_G \land PL\_2_\_G$  $\equiv$ $\lnot(TL\_1_\_G \land TL\_2_\_G)$, which is never explicitly stated in ladder logic. Note this property holds for all generated programs, irrespective of size, as shown in Figure~\ref{fig:unique_states}(b). We envisage applying such visualisations to larger interlocking programs may allow us to infer properties concerning those ladder logic programs.


\begin{figure}
	\centering
	\label{fig:unique_states}
	\subfloat[][]{
		\includegraphics[width=0.48\textwidth]{figs/p4_states.png}
	}
\hfil
	\subfloat[][]{
		\includegraphics[width=0.48\textwidth]{figs/p14_states.png}
	}
\end{figure}

\subsection{Variable Correlation}
Next we discuss the use of phi coefficients, or Matthew's Correlation Coefficient (MCC), as a basic statistical measure of association between binary variables comprising arbitrary ladder logic programs. 
For two binary variables, $(x,y)$, we compute correlation coefficient $\phi = \frac{n_{11}n_{00}-n_{10}n_{01}}{\sqrt{n_{1\bullet}n_{0\bullet}-n_{\bullet1}n_{\bullet0}}}$, where $n_{ij}$ refers to a count of observed valuations: 
 \begin{table}[]
 	\centering
	\begin{tabular}{|c|c|c|c|}
		...
		total & $n_{\bullet1}$ & $n_{\bullet0}$ & $n$     \\ \hline
	\end{tabular}
\end{table}


In other words the relation between two binary variables depends on their variability with respect to one another. If we observe all possible valuations of our program variables, i.e observe all unique reachable states, then we have sufficient samples to accurately compute this association. We populate a correlation matrix, similar to the variance-covariance matrix, for all pairs of binary variables comprising a ladder program with four coils $VAR\_(1,...,4)$, four program inputs, $ACT\_(1,...,4)$,$127$ reachable states and $2^{20}$ theoretical. Values range $[-1,1]$ where $1$ is complete correlation and $-1$ is an inverse relation.  
 
 Invariant shows no two sets of lights may simultaneously show a green aspect via inverse / positive relations (refer to the latches).

Property holds for programs with larger state space, more coils and program inputs (127 states, $2^{20}$ theoretical), correlation confirms the observed pattern in state image. See program structure, i.e variable assignment rules, in positive and inverse relations. Samples used to compute phi are observed through unique system states. Converging to an accurate estimate of phi requires maximising unique state space observations.
...
Consider Figure \ref{fig:heatmap}. Computing the phi coefficients for a set of binary variables from a generated ladder logic program with $2^{20}$ theoretical states, produces a symmetric $n \times n$ matrix $A_{\psi}$, where $n = |Val_{I} \cup Val_{C}|$. Element $a_{ij} \in A_{\psi}$ refers the phi coefficient between the $i^{th}$ row and $j^{th}$ column. Entries computing self correlation are read on the diagonal, and have been set to $0$. Given $\phi$ coefficients require an empirical sample of observations in order to determine correlation, more unique observations concerning binary variables under evaluation produces more accurate estimates of their relation. Since we wish to learn relations over massive state spaces it is likely our agents begin training with a noisy estimate of variable correlation without observing the majority of states, i.e without knowledge of all variable configurations under the reachable states, $\phi$ will remain an approximation (isn't it anyway?). 
...
%Phi Time series
%- Time series plot demonstrates how the estimate of phi refines to convergence over time steps, as new observations are made.
%- We expect complex programs with more program inputs and coils to have volatile estimates of phi due to the state combinatorics
%- This uncertainty, particularly concerning relations between coils and program inputs, may indicate infrequently explored state action pairs, likely to improve coverage.
%- It may be possible to incorporate this uncertainty in our agents reward function, issuing greater rewards for 'disproving' the current estimate of phi, i.e scale rewards according to the latest novel 		state observation's contribution to the latest estimate of phi
%- Typically phi is computed for binary pairs but one may extend this notion to include arbitrary sets of binary variables, where we may consider aribitrary formulae as individual variables in computing phi
%- One may consider the logical conjunction of two atoms under a single alias when computing phi to  say comput correlation between three variables. This could compound for all connective permutations 	such that we consider more complex invariant conditions.
...
%Using mining to verify program generation
%- Generating ground truth data by which to evaluate agent performance, programs with known state spaces were required. In order to generate such program automatically with a sufficiently complex learning objective (accoring to our reward scheme), we introduced a generative pattern of increasing depth for subsequently larger programs.
%- Looking at correlation timeseries plots wrt to act_4 we observe a sequence of increasingly positively correlated variables.
%- With each rung introduced, a new coil VAR_N is added which evaluates TRUE, iff all coils from Var_N-1 to AUDIO are also True. Similarly, an action ACT_N, introduced for every VAR_N, must be True for VAR_N to be True. These conditions also require CROSSING, PRESSED, REQ be False, which only holds for S0 of the original pelican program. Thus without sequentially selecting actions ACT_1,...,ACT_N, an agent will never reach the deepest region of our generated state space.
%- This correlation can be seen in both Figure (time series) and Figure (matrix).
%- Observe symmetrical pattern between VAR_1,...VAR_4 and ACT_1,...ACT_4
%- This, in a sense, validates the intended strcutural effect of our program generation
...
- Acknowledges need for safety guarantees in railway domain and relevance of model checking
- Introduced invariant finding problem for interlockings and how this can reduce occurrence of false positives
- Use existing approaches to translate ladder logic to propositional formulae to be modelled as an LTS which can be translated to an MDP, where reinforcement learning can be applied.
- Using RL and MDP to intelligently navigate program state space to maximise observations / state coverage
- Presented data generation (for invariant mining) technique by stacking agent trajectories during exploration
- Briefly discussed contemporary RL exploration strategies and pattern mining techniques
- Highlighted some more conventional, state of the art methods for invariant finding
- Presented set of synthetically generated programs to serve as evaluation settings where ground truth properties are known.
- Presented preliminary results on exploration strategy
- Discussed some basic invariants inferred from 
