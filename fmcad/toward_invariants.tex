...
\todo{Phil for Sunday - Overview of IC3, other invariant finding techniques for interlockings}
...
In this section we introduce a faithful mapping that given a ladder logic LTS constructs a MDP model where reinforcement learning can be applied. Hence, properties learned in the RL setting translate can be used within model checking.
...
Here we note that any unique valuation of $Val_C$ under the dynamics of a LLP constitutes a distinct state. Our action space, describes the set of ladder logic inputs used to compute new valuations after program execution. $P_a(s,s^\prime) = Pr(s_{t+1} = s^\prime | s_t, a_t)$, describes the state transition function in terms of probabilities of observing $s_{t+1}$ given action $a_t$ is taken from state $s_t$. As here, more transitions are avaliable than described by the ladder logic program, we use this probability distribution to ensure transitions match those defined by the ladder logic (essentially this will be 1 for transitions dictated by the ladder logic program and 0 for transitions that are not). 

Subsequently the environment unfolds as a set of reachable states that mirrors those of the ladder logic LTS as agents build a behaviour policy $\pi(s|a,\theta)$ according to the reward function and state transitions observed under $P_a(s,s^\prime)$. To motivate continued exploration before model performance can converge, a balance is maintained between stochastic action sampling and selecting predicted actions output by the policy network. 

Finally, our reward function and $\gamma$ can be varied in value. Aiming to maximise state space coverage we implement a reward scheme which positively rewards novel observations and avoids loops with negative reward and episode termination. In Section~\ref{results} we discuss these point further.

Consider Figure~\ref{fig_sim}...\todo{Ben: follow example description from LTS to illustrate this construction within the figure.}
...
\todo{This section needs more depth - I think we need to show the added rungs and sketch essentially how they work based upon the figure. You can also describe the formula as intuitively we aim to double the pelican...}
