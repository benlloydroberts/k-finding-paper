...
\todo{Subsec: setting up data gathering - translation, then interaction and generated trajectories}
...
\subsection{Ladder Logic Markov Decision Process}\label{subsect:MDP} \todo{Shorten and introduce concept of mining experiences to generate a dataset}
...
Consider figure \ref{fig:trajectories}. During the training process we concatenate sequences of trajectories collected across separate episodes, forming an $N\times M$ matrix $A_{\tau}$,  where $N$ refers to total observations collected during training and $M$ the number of program variables $|Val_{I} \cup Val_{C}|$, comprising any given observation. $A_\tau$ serves as our dataset from which invariants are to be mined.   
...
\todo{Briefly introduce coverage results - justify subsequent sections}
\begin{table}[!h]
...
\end{figure}
\todo{Introduce plot - how to read}
Consider Figure \ref{fig:heatmap}. Computing the phi coefficients for a set of binary variables produces a symmetric $N\times N$ matrix where element $m_{0,1}$ refers to the phi coefficient between $CROSSING$ and $REQ$. Entries computing self correlation are read on the diagonal, where they are set to $0$. 
\todo{Highlight program semantics / variable assignment, latch relations)}
\todo{Highlight generated program / artificially depth}
\todo{Add fig: correlation time series (w.r.t a variable subset)}
\todo{Introduce time series plot - how to read}
\todo{Highlight generated action correlation - suggestion of depth}
